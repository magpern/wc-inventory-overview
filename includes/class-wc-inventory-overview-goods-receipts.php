...
class WC_Inventory_Overview_Goods_Receipts {
	...
	/**
	 * Bulk reassign all goods receipts from one supplier to another (M17).
	...
	public static function reassign_supplier_bulk( int $from_id, int $to_id ) {
		...
		if ( false === $result ) {
			return new WP_Error( 'wc_io_gr_reassign_supplier_failed', 'Failed to reassign goods receipts to new supplier', array( 'db_error' => $wpdb->last_error ) );
		}
		return (int) $result;
	}
