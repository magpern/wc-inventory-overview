<?php
/**
 * Goods Receipt number allocation (M4).
 *
 * Format: GR-{YYYY}-{NNNN} (zero-padded to at least 4 digits; grows past 9999 naturally).
 * Numbers are allocated once and never reused. Mirrors WC_Inventory_Overview_PO_Numbering exactly.
 *
...
/**
 * Isolated Goods Receipt numbering service.
 */
class WC_Inventory_Overview_Goods_Receipt_Numbering {

	const OPTION_KEY  = 'wc_io_gr_number_sequence';
	const MAX_RETRIES = 3;

	/**
	 * Allocate the next Goods Receipt number for a year.
	 *
	...
	public static function allocate( $year = null ) {
		...
		while ( $attempts < self::MAX_RETRIES ) {
			...
			/**
			 * Filter the generated Goods Receipt number before uniqueness check.
			 *
			 * @param string $number Formatted receipt number.
			 * @param int    $year   Year component.
			 * @param int    $seq    Sequence component.
			 */
			$number = apply_filters( 'wc_io_gr_number', $number, $year, $seq );

			if ( ! is_string( $number ) || '' === $number ) {
				return new WP_Error( 'wc_io_gr_number_invalid', 'Filtered Goods Receipt number is empty' );
			}
		...
		return new WP_Error(
			'wc_io_gr_number_exhausted',
			sprintf( 'Unable to allocate a unique Goods Receipt number after %d attempts', self::MAX_RETRIES )
		);
	}

	/**
	 * Format a Goods Receipt number. Padding is at least 4 digits; larger sequences grow naturally.
	 *
	 * @param int $year Year.
	...
	public static function format( int $year, int $seq ): string {
		...
		return sprintf( 'GR-%d-%0' . $width . 'd', $year, $seq );
	}
	...
	/**
	 * Whether a Goods Receipt number already exists in the header table.
	 *
	 * @param string $receipt_number Receipt number.
	 */
	public static function exists( string $receipt_number ): bool {
		global $wpdb;
		$table = WC_Inventory_Overview_Goods_Receipts::table_name();
		$found = $wpdb->get_var(
			$wpdb->prepare( "SELECT id FROM {$table} WHERE receipt_number = %s LIMIT 1", $receipt_number ) // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		);
	...
	private static function normalize_year( $year ) {
		...
		if ( $year < 2000 || $year > 2100 ) {
			return new WP_Error( 'wc_io_gr_number_year', 'Goods Receipt number year out of range' );
		}
		return $year;
	}
}
