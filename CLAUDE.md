...
(and CI recovery). **M13 (Printable Purchase Order) is implemented and
frozen on `feature/m13-printable-purchase-order`, development version
`1.30.0`, opening a new unreleased feature train — not yet merged, tagged,
or published; see
[`docs/milestones/m13-implementation-plan.md`](docs/milestones/m13-implementation-plan.md)
and [`docs/checklists/m13-release-readiness.md`](docs/checklists/m13-release-readiness.md).**

## Platform status: M0–M12 published (v1.29.0); M13 frozen, unreleased
...
**Release note:** **`v1.29.0` is tagged and published** — bundled M9–M12 feature train (+ CI recovery); see [`docs/GITHUB_RELEASE_NOTES_1.29.0.md`](docs/GITHUB_RELEASE_NOTES_1.29.0.md). Intermediate development versions `1.26.0`/`1.27.0`/`1.28.0` were never tagged. Prior releases through `v1.25.0` remain tagged and published; see their respective `docs/GITHUB_RELEASE_NOTES_*.md`. **`1.30.0` (M13) is implemented and frozen but not released** — it opens a new feature train on `feature/m13-printable-purchase-order`; do not treat it as published until a future WP6 bundled release actually tags and deploys it.
