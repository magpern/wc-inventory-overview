...
(and CI recovery). **M13 (Printable Purchase Order) and M14 (Supplier Order
History) are implemented and frozen on the same unreleased feature train
(`feature/m13-printable-purchase-order` → `feature/m14-supplier-order-history`),
development version `1.31.0` — not yet merged, tagged, or published; see
...
## Platform status: M0–M12 published (v1.29.0); M13–M14 frozen, unreleased
...
**Release note:** **`v1.29.0` is tagged and published** — bundled M9–M12 feature train (+ CI recovery); see [`docs/GITHUB_RELEASE_NOTES_1.29.0.md`](docs/GITHUB_RELEASE_NOTES_1.29.0.md). Intermediate development versions `1.26.0`/`1.27.0`/`1.28.0` were never tagged. Prior releases through `v1.25.0` remain tagged and published; see their respective `docs/GITHUB_RELEASE_NOTES_*.md`. **`1.30.0` (M13) and `1.31.0` (M14) are implemented and frozen but not released** — they form one still-unreleased feature train (`feature/m13-printable-purchase-order` → `feature/m14-supplier-order-history`); do not treat either as published until a future WP6 bundled release actually tags and deploys them. The next explicit decision point is whether to continue this train with an M15 or close and release M13+M14 together.
