...
**Canonical published baseline:** `main` / tag **`v1.43.0`**
([`docs/GITHUB_RELEASE_NOTES_1.43.0.md`](docs/GITHUB_RELEASE_NOTES_1.43.0.md);
M26 freeze: [`docs/checklists/m26-release-readiness.md`](docs/checklists/m26-release-readiness.md)).
...
M21 (Position-Aware Reorder Signal) released as standalone v1.38.0, the
bundled M22–M23 Reorder-to-Draft Purchasing Workflow feature train
released as v1.40.0, the bundled M24–M25 Replenishment Planning &
Bulk Draft PO Creation feature train released as v1.42.0, and M26
(Apply Replenishment Defaults to Variations) released as standalone
v1.43.0.
...
standalone v1.38.0; M22–M23 were bundled and released as v1.40.0; M24–M25
were bundled and released as v1.42.0; M26 was released as standalone
v1.43.0**.
Intermediate development versions `1.39.0` (M22 alone) and `1.41.0` (M24
alone) were never tagged or published. See milestone plans and readiness
checklists in [`docs/milestones/`](docs/milestones/) and
[`docs/checklists/`](docs/checklists/) for complete per-milestone detail.

## Platform status: M0–M26 published (v1.43.0); M27 frozen on feature branch (v1.44.0 dev)

**Current published baseline: plugin 1.43.0, `DB_VERSION` 11.** M27 (Personal Use Stock Adjustments) is implemented and frozen on `feature/m27-personal-use-stock-adjustment` at plugin **1.44.0** / **`DB_VERSION` 12** — **not tagged or deployed**. All nine foundational
...
tagged and published as `v1.40.0`; the fifth post-GA feature train (M24–M25,
Replenishment Planning & Bulk Draft PO Creation) is tagged and published
as `v1.42.0`; **M26 (Apply Replenishment Defaults to Variations) is tagged
and published as `v1.43.0`.** **ROADMAP COMPLETE AFTER M26** — scheduled roadmap ended at M26; **M27** is the first post-roadmap milestone (frozen on feature branch, not released).
The Admin
...
delivered since that program completed; M22–M23 are the second; M24–M25
are the third.
...
**M24–M25 (Replenishment Planning & Bulk Draft PO Creation) are complete
and released together as `v1.42.0`** (merge commit
`9b7e2e83b832426296f3026334e223de403c43e0`, merged via PR #34); see
`docs/checklists/m24-release-readiness.md`/
`docs/checklists/m25-release-readiness.md` for each milestone's Level A
freeze, `docs/checklists/feature-train-m24-m25-release-readiness.md` for
the combined cross-milestone review, and
`docs/GITHUB_RELEASE_NOTES_1.42.0.md` for the published release notes.
M24 adds a read-only Purchasing → Planning screen (catalog-wide or
selection-scoped) that groups current needs-reorder items by supplier
with suggested quantities and unresolved-supplier visibility. M25 adds
explicit line selection, editable quantities, and one Draft PO per
resolved supplier group on confirm, with server-side revalidation,
advisory item locks, open/draft conflict checks, request-token replay
protection, and partial-success semantics. `PO_Service::create_draft()`
remains the sole draft-PO mutation owner; `Replenishment_Planning_Service`
remains mutation-free; zero schema change (`DB_VERSION` stays 11); zero
new public API or capability. AI-driven combined runtime acceptance (via
`wp eval-file` / `scripts/m24-m25-release-acceptance.php` against
dev.biopentra.eu, disposable fixtures, zero residue) and a code-only
rollback rehearsal (v1.42.0 → v1.40.0 → v1.42.0) were both performed
post-release with no issues found. Intermediate development version
`1.41.0` (M24 alone) was never tagged or published. M26 was later released as standalone `v1.43.0` (**ROADMAP COMPLETE AFTER M26**).
...
| M24 | Replenishment Planning Screen | ✅ Complete | 1.41.0 | [docs/milestones/m24-implementation-plan.md](docs/milestones/m24-implementation-plan.md) | Schema unchanged (v11); first milestone of the M24+M25 feature train, branched from `main`/v1.40.0. Closes the read-side bulk gap after M21–M23: a read-only, catalog-wide (or selection-scoped) Replenishment Planning screen bulk-resolves Position, supplier, and suggested quantity for every current needs-reorder item, grouped by supplier — generalizing the single-item M22 "Create Draft PO" prefill and M23 preferred-supplier/default-qty resolution to N items via the same bulk-query template M3's `get_positions_bulk()` established. New `WC_Inventory_Overview_Supplier_Preference_Resolver::decide()` (Internal, pure, INV-M24-4 sole owner of the preferred-vs-committed-history precedence rule) is extracted from and now shared by `Reorder_Prefill_Service` (refactored to delegate, external behavior byte-for-byte unchanged — proven against its own pre-existing, unmodified characterization suite) and the new `WC_Inventory_Overview_Replenishment_Planning_Service::build_plan()`. `Repository::query_products()` gains an additive `include` passthrough (characterization-proven before use: a mixed `type=>[simple,variation]` + `include` query correctly returns simple products and variations and never a variable parent — no per-ID fallback needed). `Summary::gather_low_stock_candidates()`/`get_needs_reorder_items()` extract and extend the existing gather+classify pipeline (`scan_low_stock_and_needs_reorder()` byte-identical, unmodified); truncation happens in gather order before resolution, so the inherited ~8,000-candidate catalog-wide ceiling never reaches the expensive resolution stages — resolution input is capped at 500 items catalog-wide / 100 scoped regardless of catalog size (BR-M24-21). New additive bulk-read primitives `Purchase_Order_Lines::distinct_supplier_history_for_items_bulk()` and `Replenishment_Defaults::get_bulk()` (both single-item siblings byte-for-byte unmodified) keep the resolution phase at a flat, cold-cache-measured query count (15 queries scoped at N=10/50/100; 16 queries catalog-wide at N=10/50/200, stepping to 26 at N=500 due to needing three discovery pages instead of one — not per-item scaling); `EXPLAIN` evidence at production-representative table volume confirms both supplier-history branches use indexed range scans, never a full scan or the low-selectivity `variation_id=0` key. Quantity is the configured default or `0.0`, never guessed; currency is the resolved supplier's own `default_currency`, never converted/blended. New Purchasing → Planning tab (`TAB_PLANNING`, `VIEW_PO`-gated) and Inventory Overview bulk action `wc_io_plan_replenishment` (visibility-gated on `VIEW_PO`, handler independently re-checks `VIEW_PO`, reuses the existing `bulk-wc-inventory-items` nonce, rejects selections over 100 outright, filters variable-parent selections via the same bounded `include` lookup — never a per-selected-id product load, worst-case 100×10-digit-id redirect URL proven under 2,048 chars) — both read-only, zero mutation on GET/render (M25 owns commit). No `PO_Product_Validator`/second product-load call anywhere in `build_plan()`'s resolution path (INV-M24-12). No new schema, capability, or public hook. Full pre-existing `Reorder_Prefill_Service`/`Summary`/Overview/list-table/capability-matrix regression suites re-run unmodified and stayed green throughout. Level A completion review passed; see `docs/checklists/m24-release-readiness.md`. Released as part of the M24–M25 train, `v1.42.0` (merge commit `9b7e2e83b832426296f3026334e223de403c43e0`, PR #34). Intermediate development version `1.41.0` was never tagged/published standalone. |
| M25 | Bulk Draft PO Creation | ✅ Complete | 1.42.0 | [docs/milestones/m25-implementation-plan.md](docs/milestones/m25-implementation-plan.md) | Schema unchanged (v11); second and final milestone of the M24+M25 feature train, branched from M24's frozen tip `3c21a69e6402d631575ded4653435dbaa6dbe435`. Closes the loop M24 deliberately left open: an operator reviews the Replenishment Planning screen, edits/selects quantities, and creates one Draft PO per supplier group in a single confirm action. The plugin's first multi-record mutation milestone and its first use of MariaDB advisory locks. New `WC_Inventory_Overview_Replenishment_Item_Lock` (`acquire()`/`release()`, `GET_LOCK`/`RELEASE_LOCK`, session-scoped, schema-free) serializes concurrent commits touching the same catalog item, always acquired in ascending numeric `item_post_id` order; a lock that can't be acquired within a bounded timeout is item-scoped-excluded (`concurrent_commit_in_progress`), never retried unlocked in the same request. New additive `Purchase_Order_Lines::list_open_or_draft_item_ids_bulk()` (existing sibling methods byte-for-byte unmodified) bulk-detects items already carrying a PO line in the exact frozen conflict-status set (`draft`/`placed`/`partially_received` — deliberately including `draft`; `received`/`cancelled`/`closed_short` never block), preventing an immediate retry from duplicating a line. New `WC_Inventory_Overview_Replenishment_Commit_Service::commit()` (Internal, pure orchestrator — no `$wpdb`, no `PO_Product_Validator` call, no `Replenishment_Defaults` writes, no duplicated Position/needs-reorder/supplier-precedence logic) enforces the 100-line cap, normalizes/dedupes/sorts submitted identities to a canonical `item_post_id` before any lock acquisition, rebuilds the plan with exactly one scoped `build_plan()` call (INV-M25-13), runs the bulk conflicting-line check, groups survivors by fresh `supplier_id` ascending, and calls the existing, unmodified `PO_Service::create_draft()` once per group — quantities always mapped to identities by exact canonical key, never array position. Every acquired lock is released via `finally`, unconditionally, including on a thrown exception (proven by two distinct test-only failure-injection seams, mirroring M17's `Supplier_Merge_Service` pattern: Seam A proves one group's genuine `create_draft()` failure — a supplier archived mid-commit — never affects other groups' success; Seam B proves a catastrophic mid-batch interruption leaves prior successes intact and still releases every lock, with independent-connection `IS_FREE_LOCK` evidence). New `WC_Inventory_Overview_Replenishment_Commit_Admin` (mirrors `Goods_Receipt_Admin`'s dedicated-class precedent) registers `admin_post_wc_io_replenishment_commit`: capability (`EDIT_PO`) → nonce → one-shot request token (`PO_Request_Token`, context `replenishment_commit`) → two-phase request-shape parsing (selected rows only ever validated; a selected row's invalid quantity gets a PRG notice, never `wp_die`) → delegates to `commit()` → an opaque, per-user, read-once result transient → PRG redirect. Additive-only Planning-tab commit form (`Purchasing_Page::render_planning_tab()`): every checkbox defaults unchecked, quantity editable, gated on `EDIT_PO` (a `VIEW_PO`-only viewer sees exactly M24's original read-only table). No new schema, capability, or public hook; `PO_Service::create_draft()` remains the sole draft-PO mutation owner. Frozen 100/100 measured evidence: ~1.31s, 100 created / 100 POs / 100 `GET_LOCK` / 100 `RELEASE_LOCK` / ~1,317 queries — **no `MAX_COMMIT_GROUPS` cap introduced**. Level A freeze: `docs/checklists/m25-release-readiness.md`. Released as part of the M24–M25 train, `v1.42.0` (merge commit `9b7e2e83b832426296f3026334e223de403c43e0`, PR #34). |
| M26 | Apply Replenishment Defaults to Variations | ✅ Complete | 1.43.0 | [docs/milestones/m26-implementation-plan.md](docs/milestones/m26-implementation-plan.md) | Schema unchanged (v11); standalone final scheduled roadmap milestone, branched from `main`/v1.42.0 @ `b603907`. COPY/APPLY NOW bulk Set/Clear of preferred supplier and default replenishment qty across a variable product's child variations via classic WooCommerce variation bulk-edit seams. Owner API `Replenishment_Defaults::apply_to_variations()` (`MAX_APPLY_VARIATIONS=100`), plugin-owned supplier modal + AJAX error channel, validation atomicity only (no write atomicity / no rollback), idempotent successful-operation counters, no inheritance / no parent meta / no PO-stock-cost mutation. Downstream M22/M24/M25 consume variation defaults unchanged. Level A freeze: `docs/checklists/m26-release-readiness.md`. Released standalone as `v1.43.0` via PR #35, merge commit `6b75903d549df66cad9b4f391665636318af3b10`, tag peel `96ff98956fb5e655905c286de5ef36fca8a5c052`. **ROADMAP COMPLETE AFTER M26**. |
| M27 | Personal Use Stock Adjustments | 🧊 Frozen (awaiting release) | 1.44.0 | [docs/milestones/m27-implementation-plan.md](docs/milestones/m27-implementation-plan.md) | **Schema v12** — `wc_io_stock_adjustments` / `wc_io_stock_adjustment_lines`; first post-roadmap milestone on `feature/m27-personal-use-stock-adjustment`. Audited outbound personal-use withdrawals at EUR WAC via `Stock_Adjustment_Service` → `Restock_Service` outbound mutators (ADR-0004); movement types `personal_use` / `personal_use_void`; admin tab + §4.1 reconciliation guide; no REST/CLI. Level A freeze: `docs/checklists/m27-release-readiness.md`. **Not tagged or deployed.** |

**Feature train (released):** M24 (Replenishment Planning Screen) and M25 (Bulk Draft PO Creation) were implemented, Level A frozen, and CI-green on their own feature branches (`feature/m24-replenishment-planning`, `feature/m25-bulk-draft-po-creation`, the latter branched from the former's frozen tip), then combined-reviewed (`docs/checklists/feature-train-m24-m25-release-readiness.md`, no CRITICAL/MAJOR findings) and released together as `v1.42.0` via PR #34, merge commit `9b7e2e83b832426296f3026334e223de403c43e0`, mirroring the exact precedent already used for M9–M12, M13–M15, M18–M19, and M22–M23. Draft PRs #32 (M24 alone) and #33 (M25 alone) were closed unmerged, superseded by the bundled release PR. The intermediate development version `1.41.0` (M24 alone) was never tagged or published. **M26 was released standalone as `v1.43.0`. M27 is frozen on feature branch — not released.**
