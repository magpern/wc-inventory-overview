...
**Current published baseline: plugin 1.40.0, `DB_VERSION` 11.** M24
(Replenishment Planning Screen) is implemented, Level A frozen, and
CI-green on `feature/m24-replenishment-planning` (development version
`1.41.0`, unreleased) — see the Implementation Status table below and
`docs/checklists/m24-release-readiness.md`. M25 (Bulk Draft PO Creation)
is implemented on `feature/m25-bulk-draft-po-creation`
(development version `1.42.0`, unreleased); Level A freeze evidence and
CI status are recorded in `docs/checklists/m25-release-readiness.md`
once WP2–WP4 complete. All nine foundational
