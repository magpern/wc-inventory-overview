...
**In-progress feature train:** M22 (Reorder → Draft Purchase Order Quick Action) is implemented, Level A frozen, and CI-green on its own feature branch (`feature/m22-reorder-draft-po-quick-action`) — **not yet merged to `main`, not tagged, not released**. It is the first milestone of the M22+M23 feature train; M23 will branch from M22's frozen tip, and the two will release together once M23 also freezes, mirroring the exact precedent already used for M9–M12, M13–M15, and M18–M19. The "Platform status"/"Canonical published baseline" headers above deliberately remain unchanged (`main` still reflects v1.38.0) until that combined release lands.
