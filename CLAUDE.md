...
`docs/checklists/m24-release-readiness.md`; it awaits M25 (not yet
started) for a combined feature-train release, per the same pattern used
for every prior feature train. All nine foundational
...
**Feature train (in progress, unreleased):** M24 (Replenishment Planning Screen) is implemented, Level A frozen, and CI-green on `feature/m24-replenishment-planning`, branched from `main`/v1.40.0. It will combine with M25 (Bulk Draft PO Creation, not yet started) into one feature-train release once M25 also freezes, mirroring the exact precedent used for M9–M12, M13–M15, M18–M19, and M22–M23. Development version `1.41.0` is not tagged or published while this train is in progress.
