...
- **100/100 lines/suppliers worst-case measurement** is recorded in `docs/checklists/m25-release-readiness.md` from the freeze-time rerun (not a stale pre-audit figure). Per §24's decision rule: if the measured wall time clears ~10s and lock overhead is not disproportionate, **no `MAX_COMMIT_GROUPS` cap is introduced**.
