...
- **Code rollback is unconditionally safe.** The two new controller classes and their `require_once` lines simply disappear; `Plugin` reverts to owning Overview/Restock's methods directly. No option, table, or column is introduced or altered by this milestone.
- **Business data generated by manual mutation testing (bulk actions, inline stock edits, restock/cost-adjustment submissions against disposable fixture products) must be cleaned up separately from any code rollback** — it is ordinary product/movement-ledger state, not schema/migration state, and a code rollback does not remove it.
- Like M18/M19, **M20 leaves nothing behind** beyond the absence of the two new controller classes after rollback.
