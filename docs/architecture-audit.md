...
1. **Large god class:** `class-wc-inventory-overview-plugin.php` centralizes UI, handlers, and exports — harder to test and review. **Phase 1 (M18) complete:** Dashboard and Settings tabs extracted into dedicated `WC_Inventory_Overview_Dashboard_Controller` and `WC_Inventory_Overview_Settings_Controller` classes (~42% of the original 2,706 lines moved out). **Phase 2 (M19) complete:** the three read-only reporting tabs (Movements, Order Profit, Product Profitability) extracted into a new `WC_Inventory_Overview_Reporting_Controller` (Plugin then 1,230 lines, ~55% total reduction from the original 2,706). **Phase 3 (M20) complete:** the two remaining, mutation-bearing tabs — Overview (bulk product-status/visibility/stock-status mutation + inline-stock AJAX) and Restock (two admin-post handlers writing stock/cost, plus the read-only cost-adjustment preview AJAX) — extracted into `WC_Inventory_Overview_Overview_Controller` and `WC_Inventory_Overview_Restock_Controller` respectively, Restock first (self-contained enqueue, mutation already delegated to unmodified domain services), Overview second (inline bulk-mutation code, `enqueue_assets()` split from Dashboard's). Mutation ownership unchanged throughout: `Restock_Service`/`Cost_Adjustment_Service` remain the sole, unmodified authoritative mutators; Overview's bulk/inline mutations remain inline `WC_Product` calls, relocated verbatim. **The god-class decomposition project is now complete** — Plugin is 410 lines, ~85% total reduction from the original 2,706, and owns only the composition-root/shell (menu, tab routing/dispatch, legacy redirects, Dashboard's own asset enqueue).
...
- Split `Plugin` into tab controllers or modules — **Phase 1 (M18) complete:** Dashboard + Settings extracted; **Phase 2 (M19) complete:** Movements + Order Profit + Product Profitability extracted; **Phase 3 (M20) complete:** Overview (bulk actions + inline-stock AJAX) + Restock (mutation handlers + preview AJAX) extracted. Project complete — Plugin is composition-root/shell only.
