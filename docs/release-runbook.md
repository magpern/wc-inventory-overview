...
**M17 is a schema-change / ownership-boundary-change milestone and releases standalone, not via a feature train** (this repo's own Release Triggers rule — see `docs/process/milestone-lifecycle.md`). The steps below apply once M17 is tagged and released as `v1.34.0`; they are not performed at M17's own implementation completion. After M17 freeze (WP4), the next authorized process step is the standalone release itself, only with explicit approval.

0. **Release notes file:** produce `docs/GITHUB_RELEASE_NOTES_1.34.0.md` at release time, per this repo's standard per-release convention.
1. **Schema change confirmed:** `DB_VERSION` moves `'10'` → `'11'`. Before tagging, run the schema-shape assertion against a copy of the production database (or a fresh install) and confirm it passes: `wc_io_suppliers.merged_into_supplier_id` column present, `wc_io_supplier_merges` table present with its two indexed foreign-key-by-convention columns (`source_supplier_id`, `target_supplier_id`).
2. **Pre-tag v11 schema check (mandatory):** run `WC_Inventory_Overview_Install::assert_schema_shape()` (or the equivalent WP-CLI/admin-notice surface) against the target deployment database *before* tagging — confirms the upgrade path (`maybe_upgrade()`) will fire correctly on activation of the new version, not just that a fresh test install passes.
3. **Merge tool verified end-to-end:** on the Supplier detail screen for an eligible (active or archived, not-already-merged) supplier, the "Merge into another supplier" section renders; the typed-confirmation gate correctly disables/enables the submit button; a completed merge correctly reassigns all Purchase Orders and Goods Receipts (spot-check a mixed-status source supplier), archives + permanently marks the source, and shows the correct success notice with reassignment counts.
4. **Reactivation prevention verified independently at all three layers:** (a) `Suppliers::reactivate()` returns `false` for a merged supplier, (b) the `wc_io_supplier_reactivate` admin-post handler shows an error notice (not a false success) for a merged supplier, (c) the Suppliers list-table row action shows "Merged into {target}" text, not a Reactivate link, for a merged row.
5. **Concurrent-create closure verified:** a new Purchase Order or Goods Receipt draft cannot be created against an already-merged supplier (`wc_io_po_supplier_inactive`/`wc_io_gr_supplier_inactive`); ordinary active-supplier draft creation is unaffected (no regression).
6. **Architecture guards pass:** filter includes `Test_WC_IO_Supplier_Merge_Architecture` (sole-mutator guard, no-SQL-in-admin-page guard, no-new-hooks guard) — 0 failures.
7. **Query-count contract confirmed:** the measured `merge()` query count is constant across fixture scales (already proven at 500/2,000/5,000 related POs during implementation: 11 queries at every scale) — re-confirm via `Test_WC_IO_Supplier_Merge_Performance` before tagging, do not merely trust the implementation-time measurement.
8. **Full test suite green:** unit, M1–M17-focused, and full integration suites pass with 0 failures / 0 errors / 0 risky. `--list-tests` confirms discovery of every new M17 test class.
9. **Prior surfaces unaffected:** Observed Lead Time, On-Time Rate, Order History, Spend Summary, Inventory Position drilldown, and ordinary PO/Goods Receipt creation/placement/receiving behavior unchanged.
10. **This milestone trips this repo's Release Triggers rule** (schema change + ownership-boundary change) — it does **not** wait for or join a feature train, regardless of what other milestones are in flight at release time.
11. **Pre-first-production-merge backup recommendation:** because a supplier merge is irreversible at the product level (no in-app undo — see `docs/rollback-plan.md`'s M17 note), recommend the operator take a database backup (`wp db export`, per `docs/deployment-checklist.md`) before performing the *first* production merge after this release, even though the release itself carries no special backup requirement beyond the standard pre-deploy backup already taken for the schema migration.
12. **Rollback awareness:** M17 is a schema-change milestone; a plugin-code rollback does **not** undo any merges already completed under v1.34.0+ (see `docs/rollback-plan.md`'s M17 note — the schema additions are safe to leave in place on a code rollback, matching the general M6 precedent, but the merge *operation itself* has no undo path at any code version).
